...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_retrieve_recipe(self):
        ...
        sample_recipe(user=self.user, **{"title": "test2 recipe"})

        res = self.client.get(RECIPES_URL)

        recipes = Recipe.objects.all().order_by("-id")

        serializser = RecipeSerializer(recipes, many=True)
    ...
    def test_view_recipe_detail(self):
        ...
        recipe.tags.add(sample_tag(user=self.user, name="Side course"))
        recipe.ingredients.add(sample_ingredient(user=self.user, name="Suger"))
